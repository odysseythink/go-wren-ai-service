...
func newSQLExpansionHandler(s *service.SQLExpansionService) chi.Router {
	r := chi.NewRouter()
	r.Post("/", func(w http.ResponseWriter, r *http.Request) {
		var req model.SQLExpansionRequest
	...
	r.Patch("/{query_id}", func(w http.ResponseWriter, r *http.Request) {
		...
		s.StopSQLExpansion(queryID)
		json.NewEncoder(w).Encode(model.StopSQLExpansionResponse{QueryID: queryID})
	})
	r.Get("/{query_id}/result", func(w http.ResponseWriter, r *http.Request) {
		queryID := chi.URLParam(r, "query_id")
		result := s.GetResult(queryID)
		var resp model.SQLExpansionResultResponse
